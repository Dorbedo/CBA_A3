...
class FunctionFile:
    ...
    def process_header(self):
        ...
        header_lines = self.header.strip().splitlines()
        if header_lines[-1].strip().startswith('-------------'):
            self.inputstyle = 'cba'
        elif header_lines[1].startswith(' *'):
            self.inputstyle = 'ace'

        # remove the header identifactor lines
        ...
        self.sections = re.split(r"^(Function|Parameters|Returns|Examples|Description|Name|Author|Argument|Return Value|Example|Public)s?:\s?", header_text, 0, re.M)
    ...
    def get_public(self):
        """analyzes the header for publicity"""
        if self.inputstyle == 'cba':
            return True
        # If public section is missing we can't continue
        public_text = self.get_section("Public").strip()
        if not public_raw:
    ...
    def get_description(self):
        if not self.inputstyle == 'cba':
            raw = self.get_section('Author')
            return [line.rstrip() for line in raw.splitlines(1)[1:]]
        else:
            raw = self.get_section('Description')
            return raw.splitlines()

    def get_arguments(self):
        if not self.inputstyle == 'cba':
            raw = self.get_section('Argument')
